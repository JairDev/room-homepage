...
function NavBar() {
  ...
  return(
    ...
    <header ref={(ref) => headerRef.current = ref} className="App-header">
      <div ref={(ref) => logoRef.current = ref} className="App-logo"><span>room</span></div>
      <div className="content-icon-mobile-nav">
        <span ref={(ref) => buttonHamburgerRef.current = ref} onClick={handleShowBar} className="icon-hamburger">
          <img src="icon-hamburger.svg" alt=""></img>
        </span>
        <span ref={(ref) => buttonCloseRef.current = ref} onClick={handlecloseBar} className="icon-close">
